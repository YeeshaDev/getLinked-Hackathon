...
function RegisterForm({ setShowSuccessModal }) {
  ...
  const handleRegisterForm = async (e) => {
    ...
    if (
      ...
      UserData.phone_number.length !== 11
    ) {
      setError({
        ...error,
  ...
  return (
    <div className=" lg:bg-formBg lg:p-10 lg:px-16 lg:shadow-formShadow rounded-lg flex flex-col  w-[100%] lg:w-[500px]">
      <h3 className="hidden lg:block text-purple font-semibold text-[24px] sm:text-[32px]">
        Register
      </h3>
      ...
      <p className=" block sm:!text-[22px] font-bold mt-5 text-center lg:text-start">
        CREATE YOUR ACCOUNT
      </p>
      <form action="" className="register-form flex flex-col max-w-full">
        ...
        <p className="!text-[12px] italic text-pink mt-5">
        ...
        <label className="container !text-[12px] mt-3">
          ...
          />
          <span className="checkmark"></span>I agreed with the event terms and
        ...
        <button
          ...
          className="relative z-0 bg-primaryGradient w-full lg:w-[380px]  h-12 text-white  flex items-center justify-center mx-auto  rounded-lg"
        >
          Register
        </button>
      </form>
