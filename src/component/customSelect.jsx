...
const CustomDropdown = ({handleRegisterInputChange,error}) => {
  ...
  return (
    ...
    <div className="relative hidden lg:inline-block ">
      ...
      />
      <span className='absolute top-[3.3rem] -right-[4.5rem] lg:right-2 cursor-pointer'  onClick={() => setIsOpenCategory(!isOpenCategory)} >{!isOpenCategory ? <PiCaretDownLight /> : <PiCaretUpLight />}</span>
      <ul
        className={classNames('custom-dropdown-options', {
          'absolute z-10 w-full border-t-0  bg-purple rounded-b-lg': isOpenCategory,
    ...
    <div className="relative hidden lg:inline-block">
      ...
      <input
        ...
        onChange={handleRegisterInputChange}
      />
      <span className='absolute top-[3.3rem] right-2 cursor-pointer'  onClick={() => setIsOpenSize(!isOpenSize)} >{!isOpenSize ? <PiCaretDownLight /> : <PiCaretUpLight />}</span>
      <ul
        className={classNames('custom-dropdown-options', {
          'absolute z-10 w-full border-t-0  bg-purple  rounded-b-lg': isOpenSize,
          'hidden': !isOpenSize
        })}
      >
  ...
  {/* MOBILE SELECT DROPDOWN */}
    ...
    <div className="relative inline-block sm:w-3/4 ">
      ...
      <input
        ...
        onChange={handleRegisterInputChange}
      />
      <span className='absolute top-[3.3rem] right-3  lg:right-2 cursor-pointer'  onClick={() => setIsOpenCategory(!isOpenCategory)} >{!isOpenCategory ? <PiCaretDownLight /> : <PiCaretUpLight />}</span>
      <ul
        className={classNames('custom-dropdown-options', {
          'absolute w-full z-10  border-t-0  bg-purple  rounded-b-lg': isOpenCategory,
          'hidden': !isOpenCategory
        })}
      >
